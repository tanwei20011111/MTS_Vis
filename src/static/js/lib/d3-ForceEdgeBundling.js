...
function edgeBundling() {
		...
		function compute_divided_edge_length(e_idx) {
			...
			for (let i = 1; i < subdivision_points_for_edge[e_idx].length; i++) {
				let segment_length = euclidean_distance(subdivision_points_for_edge[e_idx][i], subdivision_points_for_edge[e_idx][i - 1]);
		...
		function project_point_on_line(p, Q) {
			let L = Math.sqrt((Q.target.x - Q.source.x) * (Q.target.x - Q.source.x) + (Q.target.y - Q.source.y) * (Q.target.y - Q.source.y));
			let r = ((Q.source.y - p.y) * (Q.source.y - Q.target.y) - (Q.source.x - p.x) * (Q.target.x - Q.source.x)) / (L * L);
		...
		function apply_electrostatic_force(e_idx, i) {
			...
			for (let oe = 0; oe < compatible_edges_list.length; oe++) {
				...
				if ((Math.abs(force.x) > eps) || (Math.abs(force.y) > eps)) {
					let diff = (1 / Math.pow(custom_edge_length({
		...
		function apply_resulting_forces_on_subdivision_points(e_idx, P, S) {
			...
			for (let i = 1; i < P + 1; i++) { // exclude initial end points of the edge 0 and P+1
				let resulting_force = {
					'x': 0,
					'y': 0
				};
		...
		function compute_compatibility_lists() {
			for (let e = 0; e < data_edges.length - 1; e++) {
				for (let oe = e + 1; oe < data_edges.length; oe++) { // don't want any duplicates
